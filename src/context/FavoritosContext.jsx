...
export const FavoritosProvider = ({ children }) => {
    const [favoritosArray, setFavoritosArray, removeFavoritosArray] =
        useLocalStorage(STORAGE_KEYS.FAVORITOS, []);
    ...
    const handleMessage = useCallback((message) => {
        if (
            message.type === "FAVORITOS_UPDATE" ||
            (message.type === "STORAGE_UPDATE" &&
                message.key === STORAGE_KEYS.FAVORITOS)
        ) {
    ...
    const limpiarFavoritos = useCallback(() => {
        if (
            favoritos.size > 0 &&
            !window.confirm("¿Eliminar todos los favoritos?")
        )
            return false;
