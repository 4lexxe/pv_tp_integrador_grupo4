...
export const useProductForm = (mode, id) => {
    ...
    const { obtenerProductoPorId, agregarProducto, actualizarProducto } =
        useProductos();
    ...
    useEffect(() => {
        if (mode === "edit" && id) {
            ...
            if (producto) {
                ...
                if (!producto.esLocal) {
                    alert(
                        "Este producto viene de la API externa y no puede ser editado. Solo se pueden editar productos creados localmente."
                    );
                    navigate("/");
                    return;
                }
